...

{
  "USD_BTC": {
    "rate": 0.00001685,
    "updated_at": "2026-02-11T14:41:40"
  },
  "BTC_USD": {
    "rate": 59347.18,
    "updated_at": "2026-02-11T14:41:40"
  ...
  "last_refresh": "2026-02-11T14:41:40"
}

При вызове команды get-rate логика такая:

    - Валидируются коды валют через реестр в currencies.py (get_currency) — при неизвестном коде выбрасывается CurrencyNotFoundError.

    - Формируется ключ пары (USD_BTC) и проверяется кэш в rates.json.

    - Если курс есть и свежий, он берётся из кэша. «Свежесть» определяется TTL из настроек.

#Настройка TTL

TTL задаётся в pyproject.toml в секции [tool.valutatrade]:

[tool.valutatrade]
data_dir = "data"
rates_ttl_seconds = 300   # время жизни курса в кэше (5 минут)
base_currency = "USD"
logs_dir = "logs"
log_format = "%(asctime)s [%(levelname)s] %(name)s: %(message)s"

В коде значение читается через Singleton SettingsLoader:

    если updated_at моложе rates_ttl_seconds, курс считается свежим и используется из rates.json;

    если курс устарел или отсутствует, вызывается заглушка «Parser Service» (см. ниже), которая обновляет файл и возвращает новый курс.

Parser Service (заглушка)

В текущей версии проекта нет полноценного внешнего Parser Service — его место занимает заглушка внутри usecases.get_rate:

    для пары USD→BTC возвращается фиксированный курс 0.00001685;

    для BTC→USD — 59337.21;

    для любых других пар выбрасывается ApiRequestError с сообщением вида
    Курс USD→RUB недоступен. Повторите попытку позже.
